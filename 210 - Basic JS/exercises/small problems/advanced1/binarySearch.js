...
// console.log(binarySearch(['Alice', 'Bonnie', 'Kim', 'Pete', 'Tyler', 'Sue', 'Tyler'], 'Tyler'));  // 4

// problem
// input an array, and a search term
// output: the index position of teh search term int eh array OR -1 if not found

// In a binary search,

  // Retrieve the middle value from the data

  // If the middle value is equal to search term

  // If the middle value is less than search term discard teh lower half (including the middle)

  // if it is greater than then discard upper half including middle )

  // repeat process from the sliced half of the array.

  // repeat until the middle value is the saerch term

  // if it doesn't exist return 0

  // assume search term and elements in array are always of same type

// algorithm
// if array.length === 0 return - 1
// middle index  = array.length / 2]
// if array[middle index] is equal to search term return middle value
// otherwise:
// if teh array.middle index is greater than search key
  // call binarySearch(array slice(0, middle value + 1 ))
// if it array.middle index is less than saerch key
  // call binarySearch(array.slice(middle))
...
function binarySearch(array, key) {
  ...
  if (array[middle] > key ) {
    index -= binarySearch(array.slice(0, middle), key)
  } else {
    index +=  binarySearch(array.slice(middle), key)
  }
  return index;
}
